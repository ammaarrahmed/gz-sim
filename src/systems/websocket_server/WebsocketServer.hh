...
namespace systems
{
    ...
    /// call on the websocket. If the `<admin_authorization_key>` is set,
    /// then
    /// the connection can provide that key.
    ///
    /// * `<admin_authorization_key>` : An admin key used for
    /// authentication. If this is set, then a connection must provide the
    /// matching key using an "auth" call on the websocket. If the
    /// `<authorization_key>` is set, then the connection can provide that
    /// key.
    ///
    ...
    ///
    ///     * `<cert_file>`: Child element of `<ssl>` that contains the path
    ///       to the SSL certificate file.
    ///     * `<private_key_file>`: Child element of `<ssl>` that contains
    ///       the path to SSL private key file.
    ///
